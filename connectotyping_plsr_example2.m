...
slopes=zeros(rois,n_part); %preallocate memory to save the models
...
for i=1:n_part
    ...
    [m(:,:,i), slopes(:,i)]=make_model_plsr(signal,options); % calculate the model
end
...
for i=1:n_part %loop on participants
    for j=1:n_part % loop in model
        ...
        pred_TC=[ones(size(TC_no_AC_LS{i},1),1) TC_no_AC_LS{i}]*[slopes(:,j) m(:,:,j)]';% predicting timecourses using the model | yes, model needs to be transposed here
