...
options.rep_svd=1; % how many times to repeat the svd decomposition to maximize out of sample data predictions
...
[SV, R]=model_plsr(signal,options); %calculate the SVD
options.min_frames=size(signal,1); %how many frames to include in the model
options.SV=SV; %assign SV to options
[model slopes]=make_model_plsr(signal,options); % calculate the model

% Make predictions
pred_TC=[ones(size(signal,1),1) signal]*[slopes model]';% predicting timecourses using the model | yes, model needs to be transposed here
...
% Calculate the SVD
[SV, R]=model_plsr(signal_model,options); %calculate the SVD
...
pred_TC=[ones(size(signal_test,1),1) signal_test]*[slopes model]';% predicting timecourses using the model | yes, model needs to be transposed here
